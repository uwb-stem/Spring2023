...
function addStudents(presentationInfo, textLi, isREU) {
    ...
    if (presentationInfo.group) {               // if "group" exists in json, e.g. csse-room-6
        ...
        if (splitMajors.length > 1) {
            major.appendChild(document.createTextNode(splitMajors[0]));
            major.appendChild(brk3);
            major.appendChild(document.createTextNode(splitMajors[1]));
            major.appendChild(brk4);
            major.appendChild(document.createTextNode(splitMajors[2]));
        } else {
